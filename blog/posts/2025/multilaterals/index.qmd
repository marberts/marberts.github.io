...
Instead of reducing the dimensionality of the repeat-sales matrices, these matrices are naturally sparse when products sell infrequently (like housing) and can be stored as sparse matrices. [^1] Let's adapt the vignette from my `{rsmatrix}` package to see this in action with some data for 5 million house sales over 30 years.

[^1]: See @langsrud2023 for another example of using sparse matrices for aggregation.
